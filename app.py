...
from plots.plots import (create_unlock_plot,
create_unlock_pie_chart, create_unlock_column_chart,create_unlock_emission_plot,
 create_absolute_issuance_plot, create_relative_issuance_plot, create_issuance_ratio_plot)
...
def display_monthly_data(df: pd.DataFrame, selected_month=None):
    ...
    unlock_cols = ['Investors+Seed', 'Investors+Series', 'Team', 'Airdrop', 'Future Initiatives', 'R&D and Ecosystem', 'Total Unlock']
...
def display_yearly_data(df: pd.DataFrame, selected_year=None):
    ...
    circulating_cols = [col for col in df.columns if 'Circulating Supply %' in col or 'Cumulative' in col and 'Cumulative ' not in col]
    for col in circulating_cols:
        data[col] = year_data.iloc[-1][col]
    
    # Group columns into Unlock and Other categories
    unlock_cols = ['Investors+Seed', 'Investors+Series', 'Team', 'Airdrop', 
                  'Future Initiatives', 'R&D and Ecosystem', 'Total Unlock']
    other_cols = [col for col in monthly_cols if col not in unlock_cols]
...
def display_quarterly_data(df: pd.DataFrame, selected_quarter=None):
    ...
    # For Circulating Supply %, take the last month's value
    circulating_cols = [col for col in df.columns 
                       if 'Circulating Supply %' in col 
                       or 'Cumulative' in col and 'Cumulative ' not in col]
    for col in circulating_cols:
    ...
    for i, (name, value) in enumerate(zip(names, values)):
        ...
        with cols[col_index]:
            ...
            if isinstance(value, float):
                if name == "Annual Reward" or name == "Total Token":
                    value = f"{value:.0f}"
...
def display_unlock_sheet(df):
    ...
    years = [int(month/12+2024) for month in months if month % 12 == 0]
    ...
    with yearly_tab:
        ...
        display_yearly_data(df, (selected_year-2024))
...
def display_cumulative_data(df, months, quarters, years):
    ...
    with yearly_tab:
        ...
        display_latest_cumulative_data(df, (selected_year-2024) * 12)
...
def display_token_distribution(df, months, quarters, years):
    ...
    with yearly_tab:
        ...
        pie_fig = create_unlock_pie_chart(df, (selected_year-2024) * 12)
